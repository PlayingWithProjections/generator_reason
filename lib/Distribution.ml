...
module MonthDistribution =
  struct
    type t = {
      ...
      mutable number: int }
    type u =
      | Number of int * distribution 
      | Never 
      | ForEver of distribution 
    let create u =
      match u with
      | Never ->
          {
            distribution = ((Steady (((PerMonth (0))[@explicit_arity ])))
              [@explicit_arity ]);
            number = 0
          }
      ...
      let trueOrFalse distribution =
        ...
        | ((Spread (spread))[@explicit_arity ]) ->
            ...
            (match Map.find spread month with
             | None -> false
             | ((Some (frequency))[@explicit_arity ]) -> happens frequency) in
      if (t.number > 0) && (trueOrFalse t.distribution)
      then (t.number <- (t.number - 1); true)
      else false
  end
module PercentageDistribution =
  struct
    ...
    let add partial ~i  ~outcome  =
      ...
      {
        distributions = (((partial.currentPosition), outcome) ::
          (partial.distributions));
        currentPosition
      }
    let rest partial ~outcome  = ((partial.currentPosition), outcome) ::
      (partial.distributions)
...
let randomFromList l =
  ...
  | _ ->
      ...
      let i = Random.int length in ((Some ((List.nth_exn l i)))
        [@explicit_arity ])
let gaussianCapped ~mu  ~sigma  ~lowerBound  ~upperBound  =
  let open! Poly in
    let r =
      mu +.
        ((sigma *. (Float.sqrt ((-2.) *. (Float.log (Random.float 1.))))) *.
           (Float.cos ((2. *. Float.pi) *. (Random.float 1.)))) in
