...
describe "player answers questions" (fun { test } ->
    ...
    in
    test "fast correct player" (fun { expect } ->
        let answerType =
          { PlayerType.delay = 0.; delayRange = 0.; correctness = 1. }
        in
        let player = create answerType in
        expect.equal (`AnswerCorrectly 0.) (World.Player.answerQuestion player));
    test "fast incorrect player" (fun { expect } ->
        let answerType =
          { PlayerType.delay = 0.; delayRange = 0.; correctness = 0. }
        in
        let player = create answerType in
        expect.equal (`AnswerIncorrectly 0.)
          (World.Player.answerQuestion player));
    test "slow player" (fun { expect } ->
        let answerType =
          { PlayerType.delay = 1.; delayRange = 0.; correctness = 0. }
        in
        let player = create answerType in
