...
let createQuiz ownerId timestamp world =
  ...
  let createEvent =
    Events.create ~timestamp
      ~type_:
        (Events.QuizWasCreated { quizId; ownerId; quizTitle } [@explicit_arity])
  in
  let events, timestamp =
    List.fold_left
      ~f:(fun (events, timestamp) { World.Quiz.question; answer; id } ->
  ...
  let publishEvent =
    Events.create ~timestamp
      ~type_:(Events.QuizWasPublished { quizId } [@explicit_arity])
  in
      ...
      let events =
        ...
        | [] ->
            Events.create ~timestamp:timestampPlusFiveMinutes
              ~type_:(Events.GameWasCancelled { gameId } [@explicit_arity])
            :: events
        | players ->
            let events =
              List.fold_left
                  ...
                  Events.create
                    ...
                    ~type_:
                      (Events.PlayerJoinedGame
                         { playerId = player.World.Player.id; gameId }
                      [@explicit_arity])
                  :: events)
                ~init:events players
            in
            let events =
              Events.create ~timestamp:timestampPlusFiveMinutes
                ~type_:(Events.GameWasStarted { gameId } [@explicit_arity])
              :: events
            in
            let timestamp = timestampPlusFiveMinutes in
            let events, timestamp =
              List.fold_left
                ~f:(fun (events, timestamp) question ->
                  let questionEvent =
                    Events.create ~timestamp
                      ~type_:
                        (Events.QuestionWasAsked
                           { gameId; questionId = question.World.Quiz.id }
                        [@explicit_arity])
                  in
                  let responseEvents =
                    List.map
                      ~f:(fun player ->
                        let answerType = World.Player.answerQuestion player in
                        let timestamp =
                  ...
                  ( Events.create ~timestamp
                    ...
                    timestamp ))
                ~init:(events, timestamp) quiz.World.Quiz.questions
            in
            Events.create ~timestamp
              ~type_:(Events.GameWasFinished { gameId } [@explicit_arity])
            :: events
      in
      (world, events)
...
let createPlayerHandler timestamp world =
  if World.shouldCreatePlayer timestamp world then
...
let handleTick timestamp world =
  List.fold_left ~init:(world, [])
    ~f:(fun (world, events) handler ->
      let world, newEvents = handler timestamp world in
      (world, newEvents @ events))
    handlers

type t = { timeRange : TimeRange.t; world : World.t }
...
let run t =
  let rec run' timestamp endTimestamp events world =
    if Float.compare timestamp endTimestamp <= 0 then
  ...
  in
  run' t.timeRange.TimeRange.startTimestamp t.timeRange.TimeRange.endTimestamp
    [] t.world
