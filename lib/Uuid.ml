...
let to_string ?(upper= false)  u =
  ...
  let byte s i c =
    ...
    (Bytes.set s (i + 1)) @@ (hex hbase (c land 15)) in
  while (!j) < 4 do
    (byte s (!i) (Char.to_int (u.[!j])); i := ((!i) + 2); Int.incr j) done;
  Int.incr i;
  while (!j) < 6 do
    (byte s (!i) (Char.to_int (u.[!j])); i := ((!i) + 2); Int.incr j) done;
  Int.incr i;
...
let v4_gen seed =
  let rand = rand seed in function | () -> v4_ocaml_random_uuid rand
let generateId () =
  let randomState = Random.State.default in v4_gen randomState ()