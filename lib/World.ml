module Quiz = struct
  type quizType = Normal | Boring | Popular
  type question = { question : string; answer : string; id : Uuid.t }
  type t = { id : Uuid.t; quizType : quizType; questions : question list }
...
module Player = struct
  ...
  let create ~id ~answerType ~createQuizDistribution ~joinGameDistribution
  ...
  let answerQuestion player =
    ...
    let speed =
      Distribution.gaussianCapped
        ...
        ~lowerBound:0. ~upperBound:maximumTime
    in
    if Float.compare maximumTime speed = 0 then `AnswerTimeout
    else
      let correct =
  ...
  let shouldOpenGame timestamp player =
    Distribution.MonthDistribution.happens timestamp player.openGameDistribution

  let shouldJoinGame timestamp player =
    Distribution.MonthDistribution.happens timestamp player.joinGameDistribution
end

module Game = struct
  type t = { quiz : Quiz.t; id : Uuid.t; players : Player.t list }

  let create ~quiz ~id = { id; quiz; players = [] }
  let join ~game ~player = { game with players = player :: game.players }
end
...
let createQuiz world =
  ...
  (id, quizTitle, questions, { world with quizzes = quiz :: world.quizzes })

let playersOpeningGame timestamp world =
  List.filter_map world.players ~f:(fun player ->
      if Player.shouldOpenGame timestamp player then Some player
        [@explicit_arity]
      else None)
...
let playersCreatingQuiz timestamp world =
  List.filter_map world.players ~f:(fun player ->
      if Player.shouldCreateQuiz timestamp player then Some player
        [@explicit_arity]
      else None)
