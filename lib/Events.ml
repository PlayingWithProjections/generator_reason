...
type event = { id : Uuid.t; timestamp : float; type_ : payload }
[@@deriving show]
...
and payload =
  ...
  | QuizWasCreated of { quizId : Uuid.t; ownerId : Uuid.t; quizTitle : string }
  ...
  | QuizWasPublished of { quizId : Uuid.t }
  | GameWasOpened of { quizId : Uuid.t; gameId : Uuid.t; playerId : Uuid.t }
  | GameWasCancelled of { gameId : Uuid.t }
  | GameWasStarted of { gameId : Uuid.t }
  | PlayerJoinedGame of { playerId : Uuid.t; gameId : Uuid.t }
  | QuestionWasAsked of { gameId : Uuid.t; questionId : Uuid.t }
  | TimerHasExpired of {
      questionId : Uuid.t;
      playerId : Uuid.t;
      gameId : Uuid.t;
    }
  | GameWasFinished of { gameId : Uuid.t }
  | AnswerWasGiven of {
      questionId : Uuid.t;
      playerId : Uuid.t;
      gameId : Uuid.t;
  ...
  | QuestionWasCompleted of { questionId : Uuid.t; gameId : Uuid.t }
...
let toJson event =
  ...
  let payload =
    ...
    | ((GameWasFinished { gameId }) [@explicit_arity]) ->
        `Assoc [ ("game_id", `String (gameId |> Uuid.to_string)) ]
    | ((TimerHasExpired { gameId; questionId; playerId }) [@explicit_arity]) ->
        `Assoc
          [
            ("game_id", `String (gameId |> Uuid.to_string));
            ("question_id", `String (questionId |> Uuid.to_string));
