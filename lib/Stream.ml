let basic () =
  ...
  let playerDistribution =
    let open Distribution in
      (PercentageDistribution.empty ()) |>
        (PercentageDistribution.rest ~outcome:PlayerType.boringPlayer) in
  ...
  let events =
    (Simulation.create ~timeRange ~playerDistribution
...
let full () =
  ...
  let playerDistribution =
    let open Distribution in
      (((((PercentageDistribution.empty ()) |>
           ...
           |>
           (PercentageDistribution.add ~i:0.5
  ...
  let createPlayerDistribution =
    let m =
      (((((((((((((((((Distribution.MDistribution.init
                         timeRange.TimeRange.startTimestamp)
                        |>
                        (Distribution.MDistribution.add
                           ~data:((Distribution.PerMonth (20))
                           [@explicit_arity ])))
                       |>
                       (Distribution.MDistribution.add
                          ~data:((Distribution.PerMonth (40))
                          [@explicit_arity ])))
                      |>
                      (Distribution.MDistribution.add
                         ~data:((Distribution.PerMonth (50))
                         [@explicit_arity ])))
                     |>
                     (Distribution.MDistribution.add
                        ~data:((Distribution.PerMonth (50))
